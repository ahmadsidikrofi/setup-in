                ...
                {Object.keys(groupedOrders).map((date) => (
                    <div key={date} className="mb-6">
                        {groupedOrders[date].map((pendingOrder) => {
                            const totalHarga = pendingOrder.OrderPeripheral.reduce((total, orderPeripheral) => total + orderPeripheral.Peripherals.harga, 0)
                            return (
                                <div key={pendingOrder.id} className="border p-4 rounded-xl my-6">
                                    ...
                                    {pendingOrder.OrderPeripheral.map((orderPeripheral, i) => (
                                        <div key={i}>
