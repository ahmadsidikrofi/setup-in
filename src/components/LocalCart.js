            ...
            {localCart.map((item, i) => (
                <div key={i} >
                    <div className="xl:grid xl:grid-cols-4 lg:grid lg:grid-cols-4 md:flex md:flex-row sm:flex sm:flex-row max-sm:grid max-sm:grid-cols-4 my-5 lg:gap-x-4 md:gap-x-4 sm:gap-x-4 max-sm:gap-x-4 items-start w-full">
                        <Image src={item.image} alt="..." layout="fixed" width={150} height={150} className="sm:w-full max-sm:w-full sm:h-40  max-sm:h-24 rounded-lg object-cover" />
                        <div className="flex flex-col gap-1">
                            <Link href={`/on-your-desk/${item.slug}`}>
                                <p className="font-semibold max-sm:text-[13px] lg:text-lg sm:text-md sm:text-wrap max-sm:text-wrap">{item.nama_peripheral}</p>
                            </Link>
